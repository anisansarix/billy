...
for (let i = 0; i < ITEM_DATA.length; i++) {
    ...
    items.push({
        ...
        intraStateTaxRate: d.rate,
        interStateTaxRate: d.rate,
        salesPricePaise: d.basePrice * 100,
...
for (let i = 0; i < 150; i++) {
    ...
    for(let j=0; j<lineItemsCount; j++) {
        ...
        lineItems.push({
            ...
            igstAmountPaise: isSameState ? 0 : tax,
            cgstAmountPaise: isSameState ? tax / 2 : 0,
            sgstAmountPaise: isSameState ? tax / 2 : 0,
        });
    }
