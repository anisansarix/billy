...
export default function OutstandingList({ data }: Props) {
    ...
    const total = aging.current + aging.days1_30 + aging.days31_60 + aging.days90Plus;
    ...
    return (
        <View className="mb-6">
            ...
            <View className="h-3 flex-row rounded-full overflow-hidden mb-6 bg-slate-200">
                ...
                {aging.days31_60 > 0 && <View style={{ width: `${getWidth(aging.days31_60)}%` }} className="bg-orange-500" />}
                {aging.days90Plus > 0 && <View style={{ width: `${getWidth(aging.days90Plus)}%` }} className="bg-red-500" />}
            </View>

            {/* 2x2 Grid of Glassmorphic Cards */}
            <View className="flex-row flex-wrap justify-between">
                ...
                <View className="w-[48%] mb-4 rounded-2xl overflow-hidden border border-white/40">
                    <BlurView intensity={40} tint="light" className="p-4 bg-white/30">
                        <View className="flex-row items-center mb-2">
                            <View className="size-3 rounded-full bg-orange-500 mr-2" />
                ...
                <View className="w-[48%] mb-4 rounded-2xl overflow-hidden border border-white/40">
                    <BlurView intensity={40} tint="light" className="p-4 bg-white/30">
                        <View className="flex-row items-center mb-2">
                            ...
                            <Text className="font-sans-medium text-xs text-primary uppercase tracking-wide">60+ Days</Text>
                        </View>
