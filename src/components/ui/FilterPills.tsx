                ...
                {options.map((option) => {
                    ...
                    return (
                        <Pressable
                            ...
                            className={`px-4 py-2 rounded-full border ${isActive ? 'bg-primary border-primary' : 'bg-white border-border'} shadow-sm`}
                            style={({ pressed }) => ({ opacity: pressed ? 0.7 : 1 })}
                        >
                            <Text className={`font-sans-medium text-sm ${isActive ? 'text-white' : 'text-muted-foreground'}`}>
                                {option}
                            </Text>
                        </Pressable>
                    );
